...
gulp.task('server', ['dev'], () => {
  ...
  gulp.watch(
    [
      `${params.entry}/**/*.{jpeg,jpg,png,svg}`,
      `!${params.entry}/**/sprite*.svg`
    ],
    ['image']
  )
})
...
gulp.task('file', () => {
  gulp
    .src(`${params.entry}/**/*.{pdf,php,txt,xml}`)
    .pipe(
      rename({
        dirname: ''
      })
...
gulp.task('prod', ['dev'], () => {
  gulp
    .src(`${params.output}/css/style.css`)
    .pipe(
      postcss([
  ...
  gulp
    .src(`${params.output}/js/scripts.js`)
    .pipe(
      babel({
        presets: ['env']
      })
  ...
  gulp.src(`${params.output}/img/*`).pipe(gulp.dest(`${params.output}/img/`))
})
